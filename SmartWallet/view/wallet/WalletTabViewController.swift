...
class WalletTabViewController: TabmanViewController {
    ...
    private func getEthBalance() {
        self.rpc.getBalance(account: Application.smartwallet!.ethereumAddress) { (result) in
            switch result {
    ...
    private func get(tokenBalance: TokenBalance) {
        ...
        self.rpc.getErc20Balance(ercAddress: tokenBalance.address, account: Application.smartwallet!.ethereumAddress ) { (result) in
            switch result {
            case .success(let balance):
                DispatchQueue.main.async {
                    self.hud.dismiss()
